...
@NamedQueries(value = {
        @NamedQuery(name = "Event.selectEventAll", query = "SELECT ev FROM EventEntity ev JOIN UserSiteEntity usu ON (ev.subjectByIdSubject.siteByIdSite.idSite = usu.siteByIdSite.idSite)" +
                "WHERE usu.userByIdUser.idUser = :idUser AND ((lower(ev.subjectByIdSubject.subjectNum) " +
        ...
        @NamedQuery(name = "Event.selectEventAllNotMonitored", query = "SELECT ev FROM EventEntity ev JOIN UserSiteEntity usu ON (ev.subjectByIdSubject.siteByIdSite.idSite = usu.siteByIdSite.idSite)" +
        ...
        @NamedQuery(name = "Event.CheckNewPatient", query = "SELECT COUNT(ev) FROM EventEntity ev WHERE ev.subjectByIdSubject.idSubject = :idSubject"),
        @NamedQuery(name = "Event.CheckIcCompleted", query = "SELECT COUNT(ev) FROM EventEntity ev WHERE (ev.subjectByIdSubject.idSubject = :idSubject AND ev.formByIdForm.formNum = 20)"),
        @NamedQuery(name = "Event.CheckMois1Started", query = "SELECT COUNT(ev) FROM EventEntity ev WHERE ev.subjectByIdSubject.idSubject = :idSubject AND ev.visitByIdVisit.visitNum = 20 AND ev.formByIdForm.formNum = 10"),
        ...
        @NamedQuery(name = "Event.selectEventMedicalPermitted", query = "SELECT ev FROM EventEntity ev JOIN UserSiteEntity usu ON (ev.subjectByIdSubject.siteByIdSite.idSite = usu.siteByIdSite.idSite) " +
                "WHERE ev.formByIdForm.formNum IN (80,90) AND ((usu.userByIdUser.idUser = :idUser AND (lower(ev.subjectByIdSubject.subjectNum) LIKE CONCAT('%', :researchWord, '%')) " +
                "OR (usu.userByIdUser.idUser = :idUser AND lower(ev.formByIdForm.formLabel) LIKE CONCAT('%', :researchWord, '%')))) ORDER BY ev.subjectByIdSubject.subjectNum ASC"),
})
